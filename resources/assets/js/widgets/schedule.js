...
(function($) {
    $.fn.timeSchedule = function(options){
        ...
        this.isInRange = function(time){
            var startArr = this.setting.opening_hours['start'].split(':');
            var endArr = this.setting.opening_hours['end'].split(':');
            var start = Number(startArr[0]) * 60 * 60 + Number(startArr[1]) * 60;
            var end = Number(endArr[0]) * 60 * 60 + Number(endArr[1]) * 60;
            return this.setting.opening_hours['status'] && (time >= start && time <= end);
        };
        ...
        this.formatHour = function(min) {
            var h = "" + (min/36000|0) + (min/3600%10|0);
            var hs = (min / 3600 % 12);
            var string = ((hs == 0) ? 12 : hs) + ((h < 12) ? 'AM' : 'PM');
            return string;
        };
        ...
        this.addScheduleData = function(data){
            ...
            var max_width = setting.widthTimeX * cell_num;
            var barWidth = ((et - st) * setting.widthTimeX);
            $bar.css({
                top : ((snum * setting.timeLineY) + setting.timeLinePaddingTop),
                left : (barWidth >= max_width) ? 0 : (st * setting.widthTimeX),
                width : (barWidth >= max_width) ? max_width : barWidth,
                height : (setting.timeLineY)
            });
            //$bar.find(".time").text(stext+"-"+etext);

            if(data["title"]){
                $bar.attr("title", data["title"]);
            }
        ...
        this.addRow = function(timeline,row){
            var image_sm = row['image_sm'];
            var image_lg = row['image_lg']
            var image_title = row['image_title'];
            var title = row["title"];
            var seats = row['seats'];
            ...
            html = '';
            html += '<div class="timeline"><div class="image-frame"><a href="javascript:void(0);" class="lightbox" data-url="' + image_lg + '"><img src="' + image_sm + '" title="' + image_title + '"/></a></div></div>';
            var $data = jQuery(html);
            $data.find('.lightbox').simpleLightbox({
                sourceAttr: 'data-url',
                showCounter: false,
                nav: false,
            ...
            $element.find('.sc_footer .title').html(title);
            $element.find('.sc_footer .seats').html(seats);
            $element.find('.sc_footer .amenities').html(amenities);

            html = '';
            html += '<div class="timeline"></div>';
            var $timeline = jQuery(html);
            for(var t=tableStartTime;t<tableEndTime;t+=setting.widthTime){
                var $tl = jQuery('<div class="tl ' + ((!this.isActive() || !this.isInRange(t)) ? 'off disabled' : '')  + '"><i class="plus">&#43;</i><i class="fa fa-spinner fa-spin fa-loading"></i></div>');
        ...
        this.resizeWindow = function(){
            ...
            $element.find(".sc_footer").width(setting.dataWidth + Math.min(setting.widthTimeX*cell_num, sc_main_width));
        };
        // init
        this.init = function(){
            var html = '';
            ...
            html += '<div class="sc_wrapper_body">'+"\n";
            ...
            html += '<div class="clear" />'+"\n";
            html += '</div>'+"\n";
            html += '<div class="sc_wrapper_footer">'+"\n";
            html += '<div class="sc_footer">'+"\n";
            html += '<span class="title"></span>'+"\n";
            html += '<span class="seats"></span>'+"\n";
            html += '<span class="amenities"></span>'+"\n";
            html += '</div>'+"\n";
            html += '</div>'+"\n";
            html += '</div>'+"\n";
            ...
            for(var t=tableStartTime;t<tableEndTime;t+=setting.widthTime){
                ...
                if(
                    ...
                    var html = '';
                    html += '<div class="sc_time ' + ((!this.isActive() || !this.isInRange(t)) ? 'off disabled' : '') +  '">'+element.formatHour(t)+'</div>';
