@section('scripts')
    @parent
    {{ Html::skin('app/modules/member/guest/form.js') }}
@endsection

@section('open-tag')
{{ Form::open(array('route' => $route, 'class' => 'form-grace'))}}
@endsection
    ...
    @section('body')
        ...
        <div class="row">
            <div class="col-sm-12">
                ...
                <div class="form-group required">
                    ...
                    {{Form::text($name, isset($member) ? $member->full_name : $guest->getAttribute($field) , array('id' => $name, 'class' => 'form-control', 'maxlength' => $guest->getMaxRuleValue($field), 'title' => $translate))}}
                </div>
            </div>
        </div>
        ...
        <div class="row">
            <div class="col-sm-12">
                <div class="form-group required">
                    ...
                    {{Html::validation($guest, $field)}}
                    <label for="{{$name}}" class="control-label">{{$translate}}</label>
                    {{Form::email($name, isset($member) ? $member->{$field} : $guest->getAttribute($field) , array('id' => $name, 'class' => 'form-control', 'maxlength' => $guest->getMaxRuleValue($field), 'title' => $translate))}}
                </div>
            </div>
        </div>
        ...
        <div class="row">
            <div class="col-sm-12">
                <div class="form-group required">
                    ...
                    {{Html::validation($guest, $field)}}
                    <label for="{{$name}}" class="control-label">{{$translate}}</label>
                    {{Form::text($name, isset($member) ? sprintf('%s%s', $member->handphone_country_code, $member->handphone_number) : $guest->getAttribute($field) , array('id' => $name, 'class' => 'form-control integer-value', 'maxlength' => $guest->getMaxRuleValue($field), 'title' => $translate, 'pattern' => '^[0-9]*$'))}}
                </div>
            </div>
        </div>
        ...
        <div class="row">
            <div class="col-sm-12">
                ...
                <div class="form-group required">
                    <?php
                        $field = $guest->property()->getForeignKey();
                        $name = sprintf('%s', $field);
                        $translate = Translator::transSmart('app.Select A Location', 'Select A Location');
                    ?>
                    {{Html::validation($guest, $field)}}
                    <label for="{{$name}}" class="control-label">{{$translate}}</label>
                    {{ Form::select($name, $temp->getPropertyMenu(), (!is_null($guest->getAttribute($field))) ? $guest->getAttribute($field) : '', array('id' => $name, 'title' => $translate, 'class' => sprintf('form-control %s', $field), 'data-location-loading' => 'property-package', 'placeholder' => $translate )) }}
                </div>
            </div>
        </div>
