...
{{Html::validation($blog, 'csrf_error')}}

{{ Form::open(array('route' => $route, 'files' => true, 'class' => 'blog-form')) }}
    <div class="row">
        <div class="col-sm-3">
            <div class="photo">
                <div class="photo-frame circle lg">
                    <a href="javascipt:void(0);">
                        ...
                        ?>
                        {{ $sandbox::s3()->link($blog->profileSandboxWithQuery, $blog, $config, $dimension, ['class' => 'input-file-image-holder'])}}
            ...
            <div class="row">
                <div class="col-sm-12">
                    <div class="form-group">
                        ...
                        @endphp
                        {{Html::validation($blog, $field)}}
                        <label for="title" class="control-label">{{Translator::transSmart('app.Title', 'Title')}}</label>
                        {{ Form::text('title', $blog->title, array('id' => 'title', 'class' => 'form-control', 'title' => Translator::transSmart('app.Title', 'Title'), 'placeholder' => '')) }}
                    </div>
                </div>
                <div class="col-sm-12">
                    <div class="form-group">
                        {{Html::validation($blog, 'overview')}}
                        <label for="overview" class="control-label">{{Translator::transSmart('app.Overview', 'Overview')}}</label>
                        {{ Form::textarea('overview', $blog->overview, array('id' => 'overview', 'class' => 'form-control', 'title' => Translator::transSmart('app.Overview', 'Overview'), 'placeholder' => '')) }}
                    </div>
                </div>
            </div>
        </div>
    ...
    <div class="row">
        <div class="col-sm-12">
            <div class="form-group">
                <label for="overview">{{ Translator::transSmart("app.Blog Content", "Blog Content") }}</label>
                @php
                    $field = 'content';
                    $name = $field;
                ...
                {{Form::textarea($name, $blog->getAttribute($field) , array('id' => $name, 'class' => 'form-control', 'title' => $translate))}}
            </div>
        </div>
    </div>
    ...
    <div class="row">
        <div class="col-sm-12">
            <div class="form-group required">
                ...
                $field = 'publish';
                $name = $field;
                $translate = Translator::transSmart('app.Publish Post', 'Publish Post');
                ?>
                {{Html::validation($blog, $field)}}
                <label for="{{$name}}" class="control-label">{{$translate}}</label>
                <a href="javascript:void(0);" class='help-box' data-container="body" data-toggle="popover" data-trigger="focus"  data-placement="top" data-html="true" data-content="{{Translator::transSmart('app.Blog will never show up on front page if blog post is not in active state.', 'Blog will never show up on front page if blog post is not in active state.')}}">
                ...
                <div>
                    ...
                    {{
                        Form::checkbox(
                            $name, Utility::constant('publish.1.slug'), $blog->getAttribute($field),
                            array(
                            'data-toggle' => 'toggle',
                            'data-onstyle' => 'theme',
        ...
        <div class="col-sm-12">
            <div class="form-group required">
                ...
                <div class="input-group input-group-responsive">
                    <span class="input-group-addon">{{$meta->getPrefixCustomUrl($blog)}}</span>
        ...
        <div class="col-sm-12">
            <div class="form-group text-center">
                ...
                <div class="btn-group">
                    ...
                    {{Form::submit(Translator::transSmart('app.Cancel', 'Cancel'), array('title' => Translator::transSmart('app.Cancel', 'Cancel'), 'class' => 'btn btn-theme btn-block', 'onclick' => 'location.href="' .  URL::getLandingIntendedUrl($url_intended, URL::route('admin::blog::index', array())) . '"; return false;')) }}
                </div>
            </div>
        </div>
    </div>
