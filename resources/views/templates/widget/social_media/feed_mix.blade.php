...
@endphp
<div class="social-feed feed-mix" data-feed-id="{{$post->getKey()}}">
    <div class="top">
        <div class="profile">
            ...
            <div class="details">
                <div class="name">
                    ...
                    @if(strcasecmp($post->type, Utility::constant('post_type.1.slug')) == 0 && (!is_null($post->group)))
                        <i class="fa fa-caret-right fa-fw"></i>
                        {{Html::linkRoute('member::group::group', $post->group->name, [$post->group->getKeyName() => $post->group->getKey()], ['title' => $post->group->name])}}
                    @elseif(strcasecmp($post->type, Utility::constant('post_type.2.slug')) == 0)
                        <i class="fa fa-caret-right fa-fw"></i>
                ...
                <div class="time">
                    ...
                    @if(strcasecmp($post->type, Utility::constant('post_type.1.slug')) == 0 && (!is_null($post->group)))
                        <span>-</span>
                        <i class="fa fa-fw fa-object-group" title="{{Translator::transSmart('app.Members of :name', sprintf('Members of %s', $post->group->name), false, ['name' => $post->group->name])}}"></i>
                    @elseif(strcasecmp($post->type, Utility::constant('post_type.2.slug')) == 0)
