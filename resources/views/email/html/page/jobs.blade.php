...
@section('title', Translator::transSmart('app.New Candidate Apply For Job Vacancy', 'New Candidate Apply For Job Vacancy'))
        ...
        <tr>
            <td style="{{ $fontFamily }} {{ $style['email-body-cell'] }}">
                ...
                <p style="{{ $style['paragraph'] }}">
                    {{ Translator::transSmart('app.There is a new candidate applying for following position:', 'There is a new candidate applying for following position:') }}
                </p>
                ...
                <table style="{{ $style['table'] }}" width="100%" cellpadding="0" cellspacing="0">
                    ...
                    <col width="355">
                    <tr>
                        <td style="{{ $fontFamily }} {{ $style['table-cell'] }}" align="left">{{ Translator::transSmart('app.POSITION', 'POSITION') }}</td>
                        <td style="{{ $fontFamily }} {{ $style['table-cell'] }}" align="left">:</td>
                        <td style="{{ $fontFamily }} {{ $style['table-cell'] }}" align="left">
                            {{ $careerAppointment->career->title }}
                        </td>
                    </tr>
                    <tr>
                        <td style="{{ $fontFamily }} {{ $style['table-cell'] }}" align="left">{{ Translator::transSmart('app.JOB LOCATION', 'JOB LOCATION') }}</td>
                        <td style="{{ $fontFamily }} {{ $style['table-cell'] }}" align="left">:</td>
                        <td style="{{ $fontFamily }} {{ $style['table-cell'] }}" align="left">
                            {{ $careerAppointment->career->place }}
                        </td>
                    </tr>
                    <tr>
