...
use App\Models\MongoDB\Job as MongoDBJob;
...
class Job extends Command
{
     ...
     * @var string
     */
    protected $signature = 'recommendations:job';
     ...
     * @var string
     */
    protected $description = 'Trigger Job Recommendations';
    ...
    public function handle()
    {
        ...
        if(!$temp->isRunningJobRecommendationActivity()){

            $temp->setRunningJobRecommendationActivity();
        ...
        try {
            ...
            $cursor = MongoDBJob::raw(function($collection) use ($jobSize, $memberSize, $job, &$jobs, $today, $lastMonth)
            {
                ...
                $jobs = MongoDBJob::hydrate($results);
            ...
            foreach($jobs as $job){
                ...
                $count = $activity
                    ...
                    ->whereIn('type', [Utility::constant('activity_type.19.slug'), Utility::constant('activity_type.20.slug')])
                ...
                $isNotifyOwner = $notification_setting
                    ...
                    ->where('type', '=', Utility::constant('notification_setting.job.list.1.slug'))
                    ->where('status', '=', Utility::constant('status.1.slug'))
                    ->count();

                if($isNotifyOwner){
                    (new Activity())->add(Utility::constant('activity_type.20.slug'), $job, $job_owner , $job_owner );
                }

                $member = new Member();
                ${$member->plural()} = $member->showRandomByMatchingBio($job->job_service_matching_keys, $memberSize);

                foreach(${$member->plural()} as $member){

                    (new Activity())->add(Utility::constant('activity_type.19.slug'), $job, $job_owner , $member->getKey());
                }
            ...
            $temp->flushRunningJobRecommendationActivity();
