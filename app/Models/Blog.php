...
class Blog extends Model
{
    ...
    {
        return strlen($this->title) > 70 ? substr($this->title, 0, 70) . '...' : $this->title;
    }

    public function getShortOverviewAttribute()
    {
        return strlen($this->overview) > 150 ? substr($this->overview, 0, 150) . '...' : $this->overview;
    }
    ...
    public function showActiveOrInactiveBlog($published = true, $order = [], $paging = true)
    {
        try {
            $and = [];
            $or = [];
    ...
    public function showAll($blog_id = null, $order = [], $paging = true)
    {
        try {
            $and = [];
            $or = [];
    ...
    {
        try {
            $result = (new static())->with(['metaWithQuery'])->checkInOrFail($id);
    ...
    public function add($attributes)
    {
        try {
            ...
            $instance->getConnection()->transaction(function () use ($instance, $sandbox, $meta, $attributes) {
                ...
                // Get post attributes for meta and blogs
                ...
                // Creating meta model for blog model
                $meta->put($instance, $metaAttributes);
                $meta->assign($instance);

                // Uploading blog image profile to s3
                $config = Arr::get(static::$sandbox, 'image.profile');

                // Remove title attribute, otherwise Sandbox upload method will take
                // blog title's field and make it as 'title' field inside sandbox table
                $attributes = collect($attributes)->except('title')->all();
    ...
    public function getOneOrFailBySuffixSlug($slug)
    {
        try{
            $meta = new Meta();
            $meta = (new Meta())->with(['blog', 'blog.profileSandboxWithQuery'])
        ...
        $meta->blog->setRelation('metaWithQuery', $meta);
